...
- **Settings drawer** — ⚙ in the header. Live-tune all the layout constants (row / col gaps, container padding, node size, edge tail lengths, neck offsets — split into independent foot and body), the block + sub-node palettes, canvas background, edge / accent colors, container dash, nesting-depth opacity, CSS tokens, and the brochure / scale-grid tile scales. Reset restores defaults.
- **Type-keyed color system** — every node (outer blocks *and* the leaf modules revealed on expansion: Conv2d / BatchNorm2d / SiLU / MaxPool2d / …) draws its fill / border / text / text-size from one central, theme-aware config in `frontend/arch.jsx` (`TYPE_PALETTES`), resolved via `getNodeStyle(type, subkind)`. Expanding a block keeps the family color: the region and its nesting containers reuse the block's hue (depth shown by fill opacity), and each inner leaf takes its own type color — so an opened `Conv` reads as Conv2d → BN → SiLU in green / amber / rose, not a generic "module" gray. Only the Backbone / Neck / Head role containers are dashed; everything else is solid.
- **Light / dark theme** — ☾ button in the bottom-right zoom cluster flips everything: page chrome, panels, graph canvas, block + sub-node palettes, nesting tints, edge colors, and accents. Light is calm warm-paper with pastel blocks; dark is deep navy-slate with colored-glass fills (each block type keeps its hue, but at lower lightness with bright borders so it reads against the dark canvas instead of into it).
